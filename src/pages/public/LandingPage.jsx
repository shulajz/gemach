...
import { IMPORTANT_INFO, EVENT_TYPES, ORDER_CATEGORIES, LABELS } from '../../constants/he.js';
...
const toDateStr = (d) => (d instanceof Date ? d.toISOString().slice(0, 10) : (d || '').toString().slice(0, 10));
...
const getItemCategoryLabel = (item) => {
  if (item.orderCategoryId) {
    return ORDER_CATEGORIES.find((c) => c.id === item.orderCategoryId)?.label || '';
  }
  ...
  return ORDER_CATEGORIES.find((c) => c.id === 'tablecloths')?.label || item.category || '';
};
...
const LandingPage = () => {
  ...
  const { availabilityMap, items, loading: availabilityLoading, error: availabilityError } = useAvailability(eventDateStr);

  const tabsForEventType = useMemo(() => getTabsForEventType(form.eventType), [form.eventType]);

  const itemsForEventType = useMemo(() => {
    if (!form.eventType) return [];
    return items.filter((item) => item.category === form.eventType || item.category === 'ניטרלי');
  ...
  const itemsInCurrentTab = useMemo(() => {
    ...
    return itemsForEventType.filter((item) => itemMatchesTab(item, currentTab.id, form.eventType));
  ...
  const displayedItems = useMemo(() => {
    if (isSearching) {
      return itemsForEventType.filter((item) => matchesItemSearch(item, deferredItemSearch));
    }
  ...
  const handleChange = (field, value) => {
    ...
    if (field === 'phone') setForm((prev) => ({ ...prev, phone: formatIsraeliPhone(value) }));
  };
  ...
  const mergePendingIntoQuantities = () => {
    ...
    itemsForEventType.forEach((item) => {
      ...
      if (!Object.prototype.hasOwnProperty.call(pendingQuantities, itemId)) return;

      const qty = parseQuantityInput(normalizeQuantityDraft(pendingQuantities[itemId], maxQty), maxQty);
  ...
  const getItemsForTab = (tab) => {
    ...
    return itemsForEventType.filter((item) => itemMatchesTab(item, tab.id, form.eventType));
  };
  ...
  const handleGoToSummary = () => {
    ...
    const hasItems = itemsForEventType.some((item) => (mergedQuantities[item.id] || 0) > 0);
  ...
  const handleSubmit = async (e) => {
    ...
    const orderItems = itemsForEventType
      .map((item) => ({
        itemId: item.id,
        itemName: item.name,
        quantity: Math.min(mergedQuantities[item.id] || 0, availabilityMap[item.id]?.available ?? 0),
      }))
    ...
    return (
      <div
        ...
        className={`flex flex-col gap-4 rounded-2xl border-2 bg-white p-4 shadow-md transition-shadow sm:flex-row sm:items-center sm:gap-5 sm:p-5 ${
          inCart > 0 ? 'border-teal-400 bg-teal-50/30' : 'border-teal-200 hover:border-teal-300 hover:shadow-lg'
        }`}
      >
        <ItemImage src={item.imageUrl} alt={item.name} className="h-24 w-24 self-center rounded-xl border-2 border-teal-100 object-cover shadow sm:h-[9rem] sm:w-[9rem] sm:self-auto" />
        <div className="min-w-0 flex-1">
          <p className="text-lg font-bold text-gray-900">{item.name}</p>
          {showCategory && (
            <p className="mt-0.5 text-xs font-semibold text-teal-600">{getItemCategoryLabel(item)}</p>
          )}
          {item.notes?.trim() && (
          ...
          {inCart > 0 && (
            <p className="mt-0.5 text-sm font-semibold text-teal-800" role="status">
  ...
  return (
    <div className="space-y-6 md:space-y-10">
      <Card className="border-r-4 border-r-teal-600 bg-white shadow-lg">
        <h2 className="mb-6 text-2xl font-bold text-teal-900">{importantInfoTitle}</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        {step === 'details' && (
          <Card hover className="bg-white shadow-lg">
            ...
            <h2 className="mb-6 text-2xl font-bold text-teal-900">טופס הזמנה</h2>
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <div className="md:col-span-2">
                <span className="mb-3 block text-sm font-semibold text-gray-800">{LABELS.eventType}</span>
                    ...
                    return (
                      ...
                      >
                        <input
                          ...
                          checked={isSelected}
                          disabled={isDisabled}
                          onChange={() => !isDisabled && handleChange('eventType', type)}
                          className="sr-only"
                          aria-label={type}
                ...
                {errors.eventType && <p className="mt-1.5 text-sm font-medium text-red-600">{errors.eventType}</p>}
              </div>
            </div>
            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-end">
              <Button type="button" onClick={handleNextFromDetails} className="w-full sm:w-auto" ariaLabel={LABELS.nextCategory}>
                {LABELS.nextCategory} ←
              </Button>
            </div>
            </div>
          ...
          <>
            ...
            <div className="sticky top-4 z-30 hidden rounded-2xl border-2 border-teal-300 bg-white p-4 shadow-lg md:block">
              ...
              >
                ...
                <svg className={`h-5 w-5 flex-shrink-0 transition-transform ${cartOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {cartCount > 0 && (
                <div className="mt-3 border-t border-teal-100 pt-3">
                  ...
                  {errors.items && <p className="mt-2 text-center text-sm font-medium text-red-500">{errors.items}</p>}
                </div>
              )}
              {cartOpen && (
                <div className={`${cartCount > 0 ? 'mt-3' : 'mt-3 border-t border-teal-100 pt-3'} max-h-72 overflow-y-auto overscroll-contain`}>
                  {cartSummary.length === 0 ? (
                    <div className="rounded-xl bg-gray-50 py-8 text-center">
                      <p className="text-sm text-gray-500">עדיין לא נוספו פריטים</p>
                      <p className="mt-1 text-xs text-gray-400">בחרו קטגוריה והזינו כמות לכל פריט</p>
                    </div>
                  ) : (
                    <ul className="space-y-2" role="list">
                      {cartSummary.map(({ item, qty }) => (
                        <li key={item.id} className="flex items-center gap-3 rounded-xl border border-teal-100 bg-white p-3 shadow-sm">
                          <ItemImage src={item.imageUrl} alt="" className="h-14 w-14 flex-shrink-0 rounded-lg border border-teal-100 object-cover" />
                          <div className="min-w-0 flex-1">
            ...
            {cartOpen && cartSummary.length > 0 && (
              <div className="fixed inset-x-0 bottom-[calc(4.25rem+env(safe-area-inset-bottom,0px))] z-40 max-h-[45vh] overflow-y-auto overscroll-contain border-t border-teal-200 bg-white p-3 shadow-lg md:hidden">
                <ul className="space-y-2" role="list">
                  {cartSummary.map(({ item, qty }) => (
                    <li key={item.id} className="flex items-center gap-3 rounded-xl border border-teal-100 bg-gray-50 p-3">
                      <ItemImage src={item.imageUrl} alt="" className="h-12 w-12 flex-shrink-0 rounded-lg object-cover" />
                      <div className="min-w-0 flex-1">
            ...
            <Card hover className="bg-white pb-28 shadow-lg md:pb-8">
              ...
              {availabilityError && (
                <div className="rounded-xl border-2 border-red-200 bg-red-50 p-4 text-red-800">
                  <p className="font-medium">שגיאה בטעינת מלאי. נא לרענן את הדף.</p>
                  <p className="mt-2 text-sm">{getHebrewError(availabilityError)}</p>
                </div>
              )}
              {availabilityLoading && (
              ...
              {!availabilityLoading && !availabilityError && (
                <>
                  ...
                  <div className="mb-6 space-y-2 md:hidden">
                    {isSearching ? (
                      <div role="tabpanel" aria-label={LABELS.searchItems}>
                        ...
                        <p className="mb-3 min-h-5 break-words text-sm text-gray-600" aria-live="polite">
                        ...
                        <div className="space-y-4">
                          {displayedItems.length === 0 ? (
                            ...
                            displayedItems.map((item) => renderItemCard(item, { showCategory: true }))
                          )}
                  ...
                  <div className="hidden md:block" role="tabpanel" aria-label={isSearching ? LABELS.searchItems : currentTab?.label}>
                    <p className="mb-3 min-h-5 break-words text-sm text-gray-600" aria-live="polite">
                      {isSearching && (
                        displayedItems.length > 0
                          ? `${displayedItems.length} תוצאות עבור "${deferredItemSearch.trim()}"`
                    ...
                    >
                      {displayedItems.length === 0 ? (
                        <p className="text-gray-600">
                          {isSearching ? `לא נמצאו פריטים עבור "${deferredItemSearch.trim()}"` : 'אין פריטים בקטגוריה זו'}
                        </p>
                      ) : (
                        displayedItems.map((item) => renderItemCard(item, { showCategory: isSearching }))
                      )}
                      {errors.items && <p className="text-sm font-medium text-red-500">{errors.items}</p>}
                    </div>
                  </div>
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between sm:gap-4">
                <Button type="button" variant="secondary" className="w-full sm:w-auto" onClick={handleBackToDetails} ariaLabel={LABELS.backToDetails}>
                  → {LABELS.backToDetails}
                </Button>
                <Button type="button" disabled={submitting} className="hidden w-full shadow-lg sm:flex sm:w-auto sm:min-w-[180px]" ariaLabel={LABELS.toSummary} onClick={handleGoToSummary}>
                  {LABELS.toSummary}
                </Button>
              </div>
            </Card>
        ...
        {step === 'summary' && (
          <Card className="bg-white shadow-lg">
            <h2 className="mb-6 text-2xl font-bold text-teal-900">{LABELS.orderSummary}</h2>
            <div className="space-y-6">
              <div className="rounded-xl border border-teal-200 bg-teal-50/30 p-4">
                <h3 className="mb-3 font-semibold text-teal-900">פרטי מזמין ואירוע</h3>
                <dl className="grid gap-2 text-sm sm:grid-cols-2">
                  <div><dt className="font-medium text-gray-600">{LABELS.fullName}</dt><dd className="text-gray-900">{form.customerName}</dd></div>
                  <div><dt className="font-medium text-gray-600">{LABELS.phone}</dt><dd className="text-gray-900">{form.phone}</dd></div>
                  <div><dt className="font-medium text-gray-600">{LABELS.eventDate}</dt><dd className="text-gray-900">{form.eventDate ? form.eventDate.split('-').reverse().join('/') : ''}</dd></div>
                  <div><dt className="font-medium text-gray-600">{LABELS.city}</dt><dd className="text-gray-900">{form.city}</dd></div>
                  <div><dt className="font-medium text-gray-600">{LABELS.eventType}</dt><dd className="text-gray-900">{form.eventType}</dd></div>
                </dl>
              </div>
              <div className="rounded-xl border border-teal-200 bg-white p-4">
                <h3 className="mb-3 font-semibold text-teal-900">פריטים בהזמנה</h3>
                <ul className="space-y-2" role="list">
                  {cartSummary.map(({ item, qty }) => (
                    <li key={item.id} className="flex items-center gap-3 rounded-lg border border-gray-100 bg-gray-50/50 p-3">
                      <ItemImage src={item.imageUrl} alt="" className="h-12 w-12 flex-shrink-0 rounded-lg object-cover border border-teal-100" />
                      <div className="min-w-0 flex-1">
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between sm:gap-4">
              <Button type="button" variant="secondary" className="w-full sm:w-auto" onClick={handleBackToCategories} ariaLabel={LABELS.backToCategories}>
                → {LABELS.backToCategories}
              </Button>
              <Button type="submit" disabled={submitting} className="w-full shadow-lg sm:w-auto sm:min-w-[180px]" ariaLabel={LABELS.sendOrder}>
                {submitting ? <Spinner className="h-5 w-5" /> : LABELS.sendOrder}
              </Button>
            </div>
          </Card>
        )}
        ...
        {step === 'categories' && (!form.eventType || !form.eventDate) && (
          <p className="rounded-xl border-2 border-amber-200 bg-amber-50 p-4 font-medium text-amber-900">בחרו סוג אירוע ותאריך כדי לראות קטגוריות.</p>
        )}
      </form>
    </div>
  );
